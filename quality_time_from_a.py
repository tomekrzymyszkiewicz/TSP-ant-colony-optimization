...
def main():
    ...
    data = pd.read_csv(file_name, usecols=['graph_name', 'calculated_path',
                       'calculated_path_weight', 'defined_path_weight', 'time', 'alpha'])
    data = np.array(data)
    ...
    x = list(set(x))
    x.sort()
    y_from_alpha = {} #time,quality
    for test in data:
        if not test[5] in y_from_alpha:
            y_from_alpha[test[5]] = []
    for y in y_from_alpha:
        y_from_alpha[y] = [[100*(data[i][2]-data[i][3])/data[i][2] for i in range(len(data)) if data[i][5] == y],[data[i][4] for i in range(len(data)) if data[i][5] == y]]
    _, ax_time = plt.subplots()
    _, ax_quality = plt.subplots()
    for y in y_from_alpha:
        plt.figure(1)
        ax_time.plot(x,y_from_alpha[y][1],label=f'alfa={str(y)}',marker='o')
        plt.figure(2)
        ax_quality.plot(x,y_from_alpha[y][0],label=f'alfa={str(y)}',marker='o')
    ax_time.legend()
